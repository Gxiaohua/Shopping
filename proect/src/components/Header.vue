<template>
    <header class="header">
         ...
         <div class="md-modal modal-msg md-modal-transition" :class="{'md-show': loginState}">
        ...
        <div class="md-overlay" @click = "loginState=false" v-if= "loginState"></div>
        </header>
</template>
<script>
      ...
      methods:{
        ...
        checkLogin(){
          axios.get('/users/checkLogin').then((response)=>{
            ...
            if(res.status == '0'){
              // this.loginName = res.result  //拿到数据
              this.cartListNum()
             this.$store.commit('loginName',res.result)
            }
          })
        },
        //登录
        login(){
          ...
          axios.post('/users/login',{
             userName: this.userName,   //把登录名，密码作为参数传入
             passWord:this.passWord
          }).then((respon)=>{
              let res = respon.data
              if(res.status == "0"){
                this.errortext = false  //当登录成功时，不显示
                this.loginState = false 
                //拿到用户名
                // this.loginName = res.result.userName
                this.$store.commit('loginName',res.result.userName)
                this.cartListNum()
              }else{
                this.errortext = true   //当登录不成功时，显示
              }
          })
        },
        //登出
        ...
        //购物车总数
        cartListNum(){
          axios.get('/users/cartListNum').then((response)=>{
            let res = response.data
            if(res.status == '0'){
              // console.log(res)
