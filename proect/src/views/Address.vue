            ...
            <div class="container">
                <div class="checkout-addr">
                    ...
                    <div class="addr-list-wrap">
                        <div class="addr-list">
                        <ul>
                            <li v-for = "(item,i) in addressList_limit" 
                                :key = "item.addressId"
                                :class="{'check':checkIndex==i}"
                                @click="checkIndex = i;checkId = item.addressId"
                            >
                            <dl>
                                <dt>{{item.userName}}</dt>
                            ...
                            <div class="addr-opration addr-del">
                                <a href="javascript:;" 
                                   class="addr-del-btn"
                                    @click="delAddrItem(item.addressId)"
                                >
                    ...
                    <div class="next-btn-wrap">
                        ...
                        <router-link 
                           ...
                           :to="{path:'orderConfirm',query:{addressId:checkId}}"
                        >Next</router-link>
                    </div>
                </div>
            </div>
        </div>
        <modal :mdShow = "modalstate" @close = "changeModal">
            <p slot="message">
                确定删除吗？
            </p>
            <div slot="btnGroup">
                <a href="javascript:;" class="btn btn-m" @click="sureDel">确定</a>
                <a href="javascript:;" class="btn btn-m" @click="modalstate=false">取消</a>
            </div>
        </modal>
        <modal :mdShow = "addnew" @close = "changeModal">
            <div slot="message">
                <p>添加新地址</p>
                <ul>
                  <li class="regi_form_input">
                    <!-- <i class="icon IconPeople"></i> -->
                    <!--  v-model= "userName"   双向绑定 监听input事件 将数据和DOM进行绑定-->
                    <input type="text" tabindex="1" 
                          name="loginname"  
                          class="regi_login_input regi_login_input_left" 
                          placeholder="Name" data-type="loginname" 
                          v-model = "addname"
                    >
                  </li>
            ...
            <div slot="btnGroup">
                <a href="javascript:;" class="btn btn-m" @click="sureAdd">确定</a>
                <a href="javascript:;" class="btn btn-m" @click="addnew=false">取消</a>
            </div>
        </modal>
...
<script>
    ...
    export default {
        data(){
            return{
                checkIndex:0,   //点击时默认第一个，下标为0
                limit:3,        //限制展开条数
                addressList:[] , //接收地址数据
                modalstate:false,  //模态框的状态
                addnew:false,      //模态框的状态
                addId:'',
                checkId:'',  //选中的id
                addname:'',  //添加的名字
        ...
        methods:{
            ...
            unfoldList(){
                if(this.limit == 3){
                    //计算属性就会从新计算  this.limit = 3说明没有打开
                    this.limit = this.addressList.length
                }else{
                    // this.limit != 3 说明打开 把他变为3
                    this.limit = 3
                }
            },
            //修改默认地址
            ...
            //删除
            delAddrItem(addressId){
                 this.modalstate = true;
                 this.addId = addressId
            },
            //确定删除
            sureDel(){
                axios.post('/users/sureDel',{
            ...
            sureAdd(){
                 this.addnew = false;
                 if(this.addname == ''|| this.streetName == '' || this.tel == ''){
                     alert('请输入内容')
                     return       
                 }
                 if(isNaN(this.tel)){
                 ...
                 console.log(isNaN(this.tel))
                 axios.post('/users/addNewInner',{
